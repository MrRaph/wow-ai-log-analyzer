...
async def start_wcl_oauth(user: CurrentUser) -> WclAuthorizationStart:
    ...
    url = wcl_oauth_service.build_authorization_url(user=user)
    return WclAuthorizationStart(authorization_url=url)
...
async def wcl_oauth_callback(
    ...
    try:
        user = await wcl_oauth_service.handle_callback(session, code=code or "", state=state or "")
    ...
    return RedirectResponse(url=f"{base}/{locale}/profile?wcl=connected", status_code=302)
